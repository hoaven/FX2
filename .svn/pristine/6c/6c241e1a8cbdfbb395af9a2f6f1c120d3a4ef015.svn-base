...
function shangjia1(id) {
	if (window.confirm('你确定要上架吗？')) {
		$.post(getRootPath() + "/Manage/updateshangjia.do", {
			...
			window.location.href = getRootPath() + "/Manage/findRenthouse.do?operation=3"
		});
	} else {
		return false;
	}
...
function addrthouse() {
	$.ajax({
		...
		success : function(data) {
			...
			+ "/Manage/findRenthouse.do?operation=3"
		}
	})
}
...
function uprenthouse(id) {
	$.ajax({
		...
		success : function(data) {
			$("#rhid2").val(data.id);
			$("#sptype2").val(data.sptype);
			$("#title2").val(data.title);
			$("#address2").val(data.address);
			$("#area2").val(data.area);
			$("#zxdate2").val(data.zxdate);
			$("#transferfee2").val(data.transferfee);
			$("#renovation2").val(data.renovation);
			$("#shanpuwork2").val(data.shanpuwork);
...
function updaterthouse1() {
	$.ajax({
		data : {
			...
			area : $("#area2").val(),
			zxdate:$("#zxdate2").val(),
			transferfee:$("#transferfee2").val(),
			shanpuwork:$("#shanpuwork2").val(),
		...
		type : 'post',
		dataType : 'json',
		url : getRootPath() + '/Manage/updateRenthouse3.do',
		cache : false,
		error : function(data) {
			alert("出错了！！");
		},
