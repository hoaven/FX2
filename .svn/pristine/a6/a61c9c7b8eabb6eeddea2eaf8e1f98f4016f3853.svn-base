//先要加载接口，要在函数外，保证先加载  ak用的是我自己的
document.write('<script type="text/javascript" src="http://api.map.baidu.com/api?v=2.0&ak=uLw7rDmrS1570hZHxqokOTdI"></script>');
...
function ShowMap(objname,options){
	...
	//设定黄石市初始坐标
	var point=new BMap.Point(115.09,30.2);
	//范围为3-18级
	var zoom=this._zoom;
	...
	geolocation.getCurrentPosition(function(r){
		if(this.getStatus() == BMAP_STATUS_SUCCESS){
			...
			gc.getLocation(point, function(rs){
				...
				var addVal = addComp.province + spStr + ddComp.city + spStr + addComp.district + spStr + addComp.street + spStr + addComp.streetNumber;
				callBackFun(addVal);
			});
		}
